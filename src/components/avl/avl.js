...
import ClosedCodeCaptions from '../closedcodecaptions.js';
...
class AVLTree {
    ...
    updateHeights(root){
        ...
        else root.height = 1 + Math.max(this.updateHeights(root.left), this.updateHeights(root.right));
        return root.height;
    }

    updateBalances(root){
        root.balance = (root.left !== null ? root.left.height : 0) - (root.right !== null ? root.right.height : 0);
        root.left !== null && this.updateBalances(root.left);
        root.right !== null && this.updateBalances(root.right);
        this.animator.timeline.add({
            targets: document.getElementById(`balancenode${root.id}`),
            innerHTML: root.balance,
            easing: 'easeOutCubic',
            round: 1,
            duration: 500,
    ...
    balanceTree(root){
        if (root.left !== null) this.balanceTree(root.left);
        if (root.right !== null) this.balanceTree(root.right);
        if (root.balance > 1) {
    ...
    left_rotate(z){
        ...
        z.right = rightChild.left === null ? null : rightChild.left;
        rightChild.left = z;
    ...
    right_rotate(z){
        ...
        z.left = leftChild.right === null ? null : leftChild.right;
    }
    ...
    deleteNode(node, isSuccessor){
        ...
        if (node.left !== null && node.right !== null){
            ...
            this.animator.timeline.add({
                targets: document.getElementById(`frontcontent${node.id}`),
        ...
        if (this.root) this.updateHeights(this.root);
        if (this.root) this.updateBalances(this.root);
    }
...
function addNodeToDOM(value, count) {
    ...
    frontContent.setAttribute('id', `frontcontent${count}`);
    let text = document.createTextNode(value);
    frontContent.appendChild(text);
    let balanceDiv = document.createElement('div');
    balanceDiv.classList.add('balance-node');
    balanceDiv.setAttribute('id', `balancenode${count}`);
    let balance = document.createTextNode('2');
    balanceDiv.appendChild(balance);
    frontHighlight.appendChild(frontContent);
    frontHighlight.appendChild(balanceDiv);
...
class AVL extends Component {
    ...
    constructor (props) {
        ...
        this.state = {
            inputValue: '',
            removeValue: '',
            bst: new AVLTree(this.animator, (x) => this.setState({ currentLine: x })),
            count: 0,
            disable: true,
            numActiveNodes: 0,
            treeHeight: 0,
    ...
    async handleInputSubmit(event) {
        ...
        const success = await this.state.bst.insert(this.state.inputValue, this.state.count);
        if (!success) {
    ...
    calculateShiftX(nodeContainer) {
        ...
        const rightOverflow = Math.min(0, getWidthMidpoint(nodeContainer) - AVLTree.sizeOfSubtree(this.state.bst.root.right) * HORIZONTAL_SPACING - NODE_RADIUS);
        return Math.max(NODE_RADIUS, getWidthMidpoint(nodeContainer) - AVLTree.sizeOfSubtree(this.state.bst.root.left) * HORIZONTAL_SPACING + rightOverflow);
    }
    ...
    async handleUndoRedo(action){
        ...
        if (newState !== -1) {
            ...
            newState.state.forEach( async (value, index) => {
                await this.state.bst.insert(parseFloat(value), this.state.count + index);
                shift_x = this.calculateShiftX(document.getElementById('nodecontainer'));
            });
            shift_x = this.calculateShiftX(document.getElementById('nodecontainer'));
            this.setState({
    ...
    async tearDownTree(){
        ...
        this.setState({disable: false, bst: new AVLTree(this.animator, (x) => this.setState({ currentLine: x }))});
        return;
    }
